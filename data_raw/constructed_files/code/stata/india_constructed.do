...
********************************************************************************
	...
	* Last run:		22 May 2018
	...
	---------------+-------------------------------------------------------+----------
	  Older cohort |     1,008        994        977        952        922 |     4,853 
	Younger cohort |     2,011      1,950      1,931      1,915      1,900 |     9,707 
	---------------+-------------------------------------------------------+----------
			 Total |     3,019      2,944      2,908      2,867      2,822 |    14,560  */
...
global r1yc			N:\SurveyData\India\R1_YC\stata\
global r1oc			N:\SurveyData\India\R1_OC\stata\
global r2yc			N:\SurveyData\India\R2_YC\stata\
global r2oc			N:\SurveyData\India\R2_OC\stata\
global r3yc			N:\SurveyData\India\R3_YC\stata\
global r3oc			N:\SurveyData\India\R3_OC\stata\

global r4yc1		N:\SurveyData\India\R4_YC\YC_CH_ANON
global r4yc2		N:\SurveyData\India\R4_YC\YC_CHCOG_ANON
global r4yc3		N:\SurveyData\India\R4_YC\YC_HH_ANON
global r4oc1		N:\SurveyData\India\R4_OC\OC_CH_ANON
global r4oc2		N:\SurveyData\India\R4_OC\OC_CHCOG_ANON
global r4oc3		N:\SurveyData\India\R4_OC\OC_HH_ANON
global commid		N:\Quantitative Research\ROUNDS DEVELOPMENT\R5 Development\CAPI Programs\India\ReferenceTables\India Community List\

global r5ochh		R:\z_R5\Exported Data\India\OC_Household
global r5occh		R:\z_R5\Exported Data\India\OC_Child
global r5occog		R:\z_R5\Exported Data\India\OC_CogTest
global r5ocsaq		R:\z_R5\Exported Data\India\OC_SAQ
global r5ychh		R:\z_R5\Exported Data\India\YC_Household
global r5ycch		R:\z_R5\Exported Data\India\YC_Child
global r5yccog		R:\z_R5\Exported Data\India\YC_CogTest
global r5ycsaq		R:\z_R5\Exported Data\India\YC_SAQ

global quant		Y:\CONSTRUCTED FILES & VARIABLES
global r5calc		$quant\Calculated variables
global r5anthro		$r5calc\Anthropometrics\India
global r5wealth		$r5calc\CPI & Wealth\India
global r5location	$r5calc\LocationVars\India
global educhist		$quant\Useful variables\Education\Education history
global support		$quant\Panel R1 to R5 (in progress)\India\Supporting Data Files
global wealth		$r5calc\CPI & Wealth\India					
...
global output		$quant\Panel R1 to R5 (in progress)\India
...
***** PANEL INFORMATION *****
	...
	* ROUND 1
			use childid using "$r1yc/inchildlevel1yrold.dta", clear
			gen yc=1
			qui append using "$r1oc/inchildlevel8yrold.dta", keep(childid)
			replace yc=0 if yc==.
			gen inr1=1
			tempfile r1
			save `r1'
	...
	* ROUND 2
			use childid using "$r2yc/inchildlevel5yrold.dta", clear
			gen yc=1
			qui append using "$r2oc/inchildlevel12yrold.dta", keep(childid)
			merge 1:1 childid using "$r2oc\INChildQuest12YrOld.dta", keepusing(childid)
			replace yc=0 if yc==.
	...
	* ROUND 3
			use childid using "$r3yc/in_yc_householdlevel.dta", clear
			merge 1:1 childid using "$r3yc\IN_YC_ChildLevel.dta", keepusing(childid)
			gen yc=1
			gen inr3=1 if yc==1 & (_merge==3 | _merge==2)
			drop _m
			qui append using "$r3oc/in_oc_householdlevel.dta", keep(childid)
			merge 1:1 childid using "$r3oc\IN_OC_ChildLevel.dta", keepusing(childid)
			replace yc=0 if yc==.
	...
	* ROUND 4
			use CHILDCODE using "$r4yc1/IN_R4_YCCH_YoungerChild.dta", clear
			merge 1:1 CHILDCODE using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta", keepusing(CHILDCODE)
			gen yc=1
			gen inr4=1 if yc==1 & (_merge==3 | _merge==1)
			drop _m
			qui append using "$r4oc1/IN_R4_OCCH_OlderChild.dta", keep(CHILDCODE)
			merge 1:1 CHILDCODE using "$r4oc3\IN_R4_OCHH_OlderHousehold.dta", keepusing(CHILDCODE)
			replace yc=0 if yc==.
			replace inr4=1 if yc==0 & (_merge==3 | _merge==1)
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE _m
			tempfile r4
			save `r4'
...
***** HOUSEHOLD ADDRESS *****
	...
	* ROUND 1
			use childid typesite region dint clustid commid using "$r1yc\inchildlevel1yrold.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid typesite region dint clustid commid)
			gen round=1
			tempfile   typesiter1
			save      `typesiter1'

	* ROUND 2
			use childid typesite region dint clustid commid using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid typesite region dint clustid commid)
			gen round=2
			tempfile   typesiter2
			save      `typesiter2'

	* ROUND 3
			use childid typesite region dint clustid commid using "$r3yc\in_yc_householdlevel.dta", clear
			qui append using "$r3oc\in_oc_householdlevel.dta", keep(childid typesite region dint clustid commid)
			gen round=3
			tempfile   typesiter3
			save      `typesiter3'
			
	* ROUND 4
			use DINT CHILDCODE NEWCOMR4 NEWCLSR4 NRCOMMR4 NEWMCR4 NEWNONL4 NEWREGR4 using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta" , clear
			rename NEWNONL4 NEWSTER4  //similar to the older cohort
			replace NRCOMMR4 = string(015) if CHILDCODE==30100 //cleaning codes
			replace NRCOMMR4 = string(094) if CHILDCODE==10071
			destring NRCOMMR4, replace

			merge 1:1 CHILDCODE using "$r4oc3\IN_R4_OCHH_OlderHousehold.dta", keepusing (DINT CHILDCODE NEWCLSR4 NEWCOMR4 NRCOMMR4 NEWMCR4 NEWSTER4 NEWREGR4) nogen
			gen dint=date(DINT, "DMYhms")
			format dint %td
			drop DINT
			rename NEWREGR4 region
			
			// where YLC moved to a new community I'll fill the variable for community id with the id's of the nearby community or mini-community.
			replace NEWCOMR4 =  NEWMCR4 if NEWCOMR4==999 & NEWMCR4!=.
			replace NEWCOMR4 =  NRCOMMR4 if NEWCOMR4==999 & NRCOMMR4!=.
			replace NEWCOMR4 =. if NEWCOMR4==999 & NRCOMMR4==. & NEWMCR4==.
			rename NEWCOMR4 commid 
			rename NEWCLSR4 clustid
			destring clustid, replace
			rename NEWSTER4 typesite4 //only available for the new communities
			drop NRCOMMR4 NEWMCR4

			gen commid_r4="IN"+string(commid, "%03.0f") if commid!=.
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE
			//the clusterid needs to be cleaned for r4*
			
			tempfile comm4
			save `comm4'
			
			use "$commid\Community codes used in R2 R3 and R4.dta", clear 
			drop Community C4 C6
			gen commid_r4="IN"+string(commid, "%03.0f")
			drop commid
			encode Urban_Rural, gen (typesite)
			drop Urban_Rural	
			gen type=1 if typesite==2|3
			replace type=2 if typesite==1
			drop typesite
			merge 1:m commid_r4 using `comm4'	
			tab _m	
			br if _m==1  //community id's but no corresponding childid  	
			drop if _m==1
			replace type= typesite4 if type==.  //replace typesite for those in new new communities which we have information on typesite//	
			drop typesite4 _m commid
			rename type typesite 
			rename commid_r4 commid
			gen round=4
			tempfile typesiter4
			save    `typesiter4'

	* ROUND 5
			use "$r5location\in_r5_yc_hh_location.dta", clear
			qui append using "$r5location\in_r5_oc_hh_location.dta"
			g round=5
			rename *_hh *
			tempfile location5
			save `location5'
			...
			g childid="IN"+string(CHILDCODE,"%06.0f")
...
***** CHILD LOCATION *****
	...
	* ROUND 2 
			use childid id livhse using "$r2yc\INSubHouseholdMember5.dta", clear
			qui append using "$r2oc\INSubHouseholdMember12.dta", keep(childid id livhse)
			keep if id==0
	...
	* ROUND 3		
			use childid id livhse using "$r3yc\IN_YC_HouseholdMemberLevel.dta", clear
			qui append using "$r3oc\IN_OC_HouseholdMemberLevel.dta", keep(childid id livhse)
			keep if id==0
	...
	* ROUND 4
			use CHILDCODE MEMIDR4 LIVHSER4 using "$r4yc3\IN_R4_YCHH_HouseholdRosterR4.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_HouseholdRosterR4.dta", keep(CHILDCODE MEMIDR4 LIVHSER4)
			keep if MEMIDR4==0
	...
	* ROUND 5
			...
			g round=5
			duplicates drop
			drop if childid=="IN140027" & LIVHSER5==3
			keep childid childloc round
			tempfile childlocr5
			save `childlocr5'	
	...
	* LABELS
			...
			label define typesite 1 "urban" 2 "rural" 77 "Not known"
			label values typesite typesite			

			recode region (99=.)
			label define region 21 "Coastal Andhra" 22 "Rayalaseema" 23 "Telangana" 24 "Others" 77 "Not known"
...
***** GENERAL *****			
	...
	* SEX, ETHNICITY AND RELIGION - (taken from R1)
			use childid sex chldeth chldrel using "$r1yc/inchildlevel1yrold.dta", clear
			qui append using "$r1oc/inchildlevel8yrold.dta", keep(childid sex chldeth chldrel)
			gen chethnic=chldeth
			replace chethnic = 100 if chldeth ==24 & chldrel ==04
			replace chethnic = 101 if chldeth ==24 & chldrel ==02
			replace chethnic = 102 if chldeth ==24 & chldrel ==03
			replace chethnic = 103 if chldeth ==24 & chldrel ==07
			replace chethnic = 104 if chldeth ==24 & chldrel ==01
			drop chldeth
			rename sex chsex
			tempfile childchar
			save    `childchar'			
	...
	* FIRST LANGUAGE (Taken from R2 - NO INFORMATION IN OTHER ROUNDS)
			use childid chlng1st using "$r2yc/inchildlevel5yrold.dta", clear
			qui append using "$r2oc/inchildlevel12yrold.dta",keep(childid chlng1st)
	...
	* AGE IN MONTHS - For each round
			...
			* ROUND 1
			use childid agechild using "$r1yc/inchildlevel1yrold.dta", clear
			qui append using "$r1oc/inchildlevel8yrold.dta", keep(childid agechild)
			gen round=1
			...
			* ROUND 2
			use childid agechild using "$r2yc/inchildlevel5yrold.dta", clear
			qui append using "$r2oc/inchildlevel12yrold.dta", keep(childid agechild)
			gen round=2
			rename agechild agemon
			tempfile    age2
			save       `age2'
			...
			* ROUND 3
			use childid agechild using "$r3yc/in_yc_householdlevel.dta", clear
			qui append using "$r3oc/in_oc_householdlevel.dta", keep(childid agechild)
			gen round=3
			rename agechild agemon
			tempfile    age3
			save       `age3'
			...
			* ROUND 4
			use CHILDCODE agemon using "$r4yc1/IN_R4_YCCH_YoungerChild.dta", clear
			qui append using "$r4oc1/IN_R4_OCCH_OlderChild.dta", keep(CHILDCODE agemon)
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE			
			gen round=4
			tempfile    age4
			...
			* ROUND 5			
			use CHILDCODE agemons using "$r5anthro\in_r5_anthro_yc_indexch.dta", clear
			qui append using "$r5anthro\in_r5_anthro_oc_indexch.dta", keep(CHILDCODE agemons)
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE
	...
	* LABEL VARIABLES
			...
			label define ethniclabel 21"SC" 22"ST" 23"BC" 24 "Other" 99 "NK" 104"Other, Christian" ///
									100"Other, Hindu" 101"Other, Muslim" 102"Other, Buddhist" 14"None"
			label values chethnic ethniclabel
...
***** ANTHROPOMETRIC MEASURES *****			
	...
	* ROUND 1
			use childid chheght chweght bmi-fbfa using "$r1yc\INChildLevel1YrOld.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid chheght chweght bmi-fbfa)
			g round=1
	...
	* ROUND 2
			use childid chheight chweight bmi-fbfa zwfl fwfl using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid chheight chweight bmi-fbfa)
			g round=2
			tempfile anthrop2
			save `anthrop2'	
	
	* ROUND 3
			use childid chwghtr3 chhghtr3 using "$r3yc/in_yc_householdlevel.dta", clear
			qui append using "$r3oc/in_oc_householdlevel.dta", keep(childid chwghtr3 chhghtr3)
			rename chwghtr3 chweight
			rename chhghtr3 chheight	
			tempfile height3
			save `height3'
	
			use childid bmi-fbfa using "$r3yc\in_yc_childlevel.dta", clear
			qui append using "$r3oc\in_oc_childlevel.dta", keep(childid bmi-fbfa)
			g round=3
	...
	* ROUND 4
			use CHILDCODE CHHGTAGR4 CHWGTAGR4 using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta", clear
			merge 1:1 CHILDCODE	using "$r4yc1\IN_R4_YCCH_YoungerChild.dta", nogen keepusing(CHILDCODE bmi-fbfa)
			qui append using "$r4oc1\IN_R4_OCCH_OlderChild.dta", keep(CHILDCODE CHHGTAGR4 CHWGTAGR4 bmi-fbfa)
	...
	* ROUND 5
			use "$r5anthro\in_r5_anthro_yc_indexch.dta", clear
			qui append using "$r5anthro\in_r5_anthro_oc_indexch.dta"
	...
	* LABEL VARIABLES
			...
			drop if bmi==. & zwfa==. & zhfa==. & zbfa==. & zwfl==. & fwfa==.               
			sort childid round
			order childid round
			tempfile anthrop
			save `anthrop'			
...
***** BIRTH AND IMMUNIZATION *****
	...
	* ROUND 1 (YC ONLY)	
			use childid bwght bwdoc antnata numante inject docbrth-othbrth bcg measles polio using "$r1yc\inchildlevel1yrold.dta", clear
	...
	* ROUND 2
			use childid bcg measles dpt opv hib using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid bcg measles dpt opv hib)
			rename opv polio
			g round=2
			tempfile vaccine
			save `vaccine'
...
***** ILLNESS, INJURY, AND DISABILITY *****
	...
	* ROUND 1
			use childid mightdie longterm using "$r1yc\inchildlevel1yrold.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid mightdie longterm) 
	...
	* ROUND 2
			use childid mightdie longterm using "$r2yc\inchildlevel5yrold.dta", clear
			merge 1:m childid using "$r2yc\insubillnesses5.dta", nogen keepusing(childid illid)
			drop if illid>1 & illid!=.
			merge 1:m childid using "$r2yc\insubinjuries5.dta", nogen keepusing(childid injid)
			drop if injid>1 & injid!=.
			tempfile yc2
			save `yc2'
			use childid mightdie longterm using "$r2oc\inchildlevel12yrold.dta", clear
			merge 1:m childid using "$r2oc\insubillnesses12.dta", nogen keepusing(childid illid)
			drop if illid>1 & illid!=.
			merge 1:m childid using "$r2oc\insubinjuries12.dta", nogen keepusing(childid injid)
			drop if injid>1 & injid!=.
	...
	* ROUND 3
			use childid tminjr3 using "$r3yc/in_yc_householdlevel.dta", clear
			rename tminjr3 nmtminr3 
			qui append using "$r3oc/in_oc_childlevel.dta", keep(childid nmtminr3)
	...
	* ROUND 4
			use CHILDCODE ILLNSSR4 TMINJR4 DSBWRKR4 HOWAFTR4 using "$r4yc3/IN_R4_YCHH_YoungerHousehold.dta", clear
			rename TMINJR4 NMTMINR4
			qui append using "$r4oc1/IN_R4_OCCH_OlderChild.dta", keep(CHILDCODE ILLNSSR4 NMTMINR4 LNGHL* DSBWRKR4 HOWAFTR4)
...
***** SMOKING, DRINKING, AND REPRODUCTIVE HEALTH *****
	...
	* ROUND 3 (OLDER COHORT ONLY)
			use childid agecigr3 oftsmkr3 youalcr3 prgfrsr3-hivsexr3 using "$r3oc\IN_OC_ChildLevel.dta", clear
			...
			g round=3
			keep childid round chsmoke chalcohol chrephealth1 chrephealth2 chrephealth3
			order childid round chsmoke chalcohol chrephealth1 chrephealth2 chrephealth3 
			tempfile smoke3
			save `smoke3'

	* ROUND 4 (OLDER COHORT ONLY)
			use CHILDCODE YOUALCR4 AGECIGR4 OFTSMKR4 using "$r4oc1\IN_R4_OCCH_OlderChild.dta", clear
			merge 1:1 CHILDCODE using "$r4oc2\IN_R4_OCCOG_OlderChild.dta", nogen keepusing(CHILDCODE SELFR401-SELFR405)
			recode YOUALCR4 AGECIGR4 OFTSMKR4 SELFR401-SELFR405 (77 79 88 99=.)
			g chalcohol=YOUALCR4==1 | YOUALCR4==2 if YOUALCR4!=.
			rename OFTSMKR4 chsmoke
			replace chsmoke=5 if AGECIGR4==5
	...
	* ROUND 5
			use CHILDCODE SMOKER5 OTHRTBR5 DYSSMKR5 YOUALCR5 using "$r5ycch\YoungerChild.dta", clear
			merge 1:1 CHILDCODE using "$r5ycsaq\YoungerChildSAQ.dta", nogen keepusing(CHILDCODE PRGFRSR5-WHRCNDR5)
			recode SMOKER5 DYSSMKR5 YOUALCR5 WHRCNDR5 PRGFRSR5 WSHAFTR5 USECNDR5 LKSHLTR5 HIVSEXR5 (77 79 88 99=.)
			...
			drop if CHILDCODE==80064
			recode OFTSMKR5 YOUALCR5 WHRCNDR5 (77 79 88 99=.)
...
***** SUBJECTIVE HEALTH AND WELL-BEING *****
		...
		* ROUND 1
			use childid healthy using "$r1yc\inchildlevel1yrold.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid healthy)
			recode healthy (99=.)
		...
		* ROUND 2
			use childid healthy using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\INChildLevel12YrOld.dta", keep(childid healthy)
			merge 1:1 childid using "$r2oc\INChildQuest12YrOld.dta", nogen keepusing(childid cladder)
			rename healthy chhrel
			g round=2
			tempfile shealth2
			save `shealth2'
		...
		* ROUND 3
			use childid nmehltr3 using "$r3yc/in_yc_householdlevel.dta", clear
			merge 1:1 childid using "$r3yc/in_yc_childlevel.dta", nogen keepusing(childid stnprsr3)
			rename nmehltr3 yrhlthr3
			qui append using "$r3oc/in_oc_childlevel.dta", keep(childid yrhlthr3 stnprsr3)
		...
		* ROUND 4
			use CHILDCODE NMEHLTR4 using "$r4yc3/IN_R4_YCHH_YoungerHousehold.dta", clear
			merge 1:1 CHILDCODE using "$r4yc1/IN_R4_YCCH_YoungerChild.dta", nogen keepusing(CHILDCODE STNPRSR4)
			rename NMEHLTR4 YRHLTHR4
			rename STNPRSR4 LADNOWR4
			qui append using "$r4oc1\IN_R4_OCCH_OlderChild.dta", keep(CHILDCODE YRHLTHR4 LADNOWR4)
...
***** TIME USE *****
	...
	* ROUND 2 (NOTE: missing values for R2 mean that child is younger than 5)
			use childid id age sleep chcare hhchore npaywork paywork school study play using "$r2yc\insubhouseholdmember5.dta", clear
			keep if id==0
			...
			tempfile timeuse2yc
			save    `timeuse2yc'
			use childid csleep-cplay using "$r2oc\INChildQuest12YrOld.dta", clear
	...
	* ROUND 3
			use childid id age sleepr3-playr3 using "$r3yc/in_yc_householdmemberlevel.dta", clear
			recode *r3 (-77 77 88=.)			
			keep if id==0
			drop if age==.
			...
			tempfile timeuse3yc
			save    `timeuse3yc'			
			use childid sleepr3-lsurer3 using "$r3oc/in_oc_childlevel.dta", clear
			recode *r3 (-77 77 88=.)
	...
	* ROUND 4
			use CHILDCODE SLEEPR4-LSURER4 TMCMWKR4 TMCMSCR4 using "$r4yc1\IN_R4_YCCH_YoungerChild.dta", clear 
			rename TMCMSCR4 COMSCHR4 
			qui appen using "$r4oc1\IN_R4_OCCH_OlderChild.dta", keep(CHILDCODE SLEEPR4-LSURER4 TMCMWKR4 COMSCHR4)
			recode *R4 (-77 -88 -79=.)
			recode SLEEPR4-LSURER4 (77=.)
...
***** EDUCATION VARIABLES *****			
		...
		* Age start of schooling 
			use "$educhist\Older cohort\Data\educhistory_in_oc.dta", clear
			qui append using "$educhist\Younger cohort\Data\educhistory_in_yc.dta"	
		...
		* Pre-primary school
			use childid grade completeinfo using "$educhist\Older cohort\Data\educhistory_in_oc.dta", clear
			qui append using "$educhist\Younger cohort\Data\educhistory_in_yc.dta", keep(childid grade completeinfo)	
	...
	* TRAVEL TIME TO SCHOOL 
			...
			* ROUND 2 
			use childid tmschmin using "$r2oc/inchildquest12yrold.dta", clear
			qui append using "$r2yc/inchildlevel5yrold.dta", keep(childid tmschmin)
			...
			* ROUND 3
			use childid schminr3 using "$r3oc/in_oc_childlevel.dta", clear
			qui append using "$r3yc/in_yc_householdlevel.dta", keep(childid schminr3)
			...
			* ROUND 4 (YC only)
			use CHILDCODE SCHMINR4 using "$r4yc1/IN_R4_YCCH_YoungerChild.dta", clear
	...
	* Highest grade achieved 
			...
			* ROUND 1 (OC only; source: education history)
				/* If enrolled in 2002, hghgrade=current grade minus 1. 			
					...
					enrolled in 2002 - 948 obs
					not enrolled in 2002 & never enrolled before 2002 - 22 obs
					not enrolled in 2002 but enrolled before 2002 - 7 obs		
					missing info - 31										
			...
			replace hghgrade=3 if childid=="IN051034"
			replace hghgrade=1 if childid=="IN181026"
			replace hghgrade=. if childid=="IN181038"
			replace hghgrade=3 if childid=="IN181043"	
			replace hghgrade=2 if childid=="IN191025"	
			replace hghgrade=2 if childid=="IN191050"	
			replace hghgrade=1 if childid=="IN201005"	
			keep if year==2002
			...
			* ROUND 2 (source: household)
			...
			use childid id chgrade using "$r2oc\INSubHouseholdMember12.dta", clear
			qui append using "$r2yc\INSubHouseholdMember5.dta", keep(childid id chgrade)
			keep if id==0
			drop id
			merge 1:1 childid using "$r2yc\INChildLevel5YrOld.dta", nogen keepusing(childid hasstrt)
			...
			* ROUND 3 (source: household)
			use childid id grader3 hsstrtr3 using "$r3oc\IN_OC_HOuseholdMemberLevel.dta", clear
			qui append using "$r3yc\IN_YC_HouseholdMemberLevel.dta", keep(childid id grader3 hsstrtr3)
			...
			* ROUND 4 (source: household)
			use CHILDCODE MEMIDR4 GRADER4 GRDE18R4 HSSTRTR4 using "$r4oc3\IN_R4_OCHH_HouseholdRosterR4.dta", clear
			replace GRADER4=GRDE18R4 if missing(GRADER4)
			drop GRDE18R4
			qui append using "$r4yc3\IN_R4_YCHH_HouseholdrosterR4.dta", keep(CHILDCODE MEMIDR4 GRADER4 HSSTRTR4)
			recode GRADER4 (13 14=13) (15 16=14) (18=30)
			keep if MEMIDR4==0
			drop MEMIDR4
			...
			* ROUND 5 (source: educ history) 
			...
			recode grade (13 14=13) (15=14) (16=15) (77 100=.)
			duplicates drop
			g round=5
			...
			* MERGE
			...
			label define educ  0 "None" ///
							  ...
							  11 "Grade 11" ///
							  12 "Grade 12" ///
							  13 "Vocational, technical college" ///
							  14 "University" ///
...
***** READING AND WRITING *****
	...
	* ROUND 1 (OC only)
			...
			use childid levlread levlwrit using "$r1oc/inchildlevel8yrold.dta", clear
	...
	* ROUND 2 (OC only)
			...
			use childid levlread levlwrit using "$r2oc/inchildquest12yrold.dta", clear
			recode levlread levlwrit (77 79 88 99=.)
			g literate=levlread==4 & levlwrit==3 if levlread!=. & levlwrit!=. 	 // Reads sentence & writes sentences w/o difficulties
			g round=2
			tempfile read2
			save `read2'		
	...
	* ROUND 3 (YC only)
			...
			use childid vrbitm01 vrbitm02 using "$r3yc/in_yc_childlevel.dta", clear
			recode vrbitm01 vrbitm02 (77 79 88 99=.)
...
***** PARENT CHARACTERISTICS (FATHER) *****
	...
	* ROUND 1
			use childid id age sex relate grading using "$r1yc\insubsec2householdroster1.dta", clear
			qui append using "$r1oc\insubsec2householdroster8.dta", keep(childid id age sex relate grading)
			gen father=1 if relate==1 & sex==1
			keep if father==1                                   
			recode relate grading (88 99=.)
			recode grading (13/99=.)
			rename id dadid
			rename age dadage
			rename grading dadedu
			keep childid dad*
			tempfile  dad1
			save     `dad1'						

			use childid daddead using "$r1yc\inchildlevel1yrold.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid daddead)
	...
	* ROUND 2 (1 duplicate: IN051002 dadid=3)
			use  childid id memsex livhse grade age relate using "$r2yc\insubhouseholdmember5.dta", clear
			qui append using "$r2oc\insubhouseholdmember12.dta", keep(childid id memsex age relate livhse grade)
			...
			drop if childid=="IN051002" & dadid==3
			keep childid dad*
			tempfile  dad2
			save     `dad2'

			use childid dadlits using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid dadlits)
	...
	* ROUND 3 (1 duplicate: IN081018 dadid==0)
			use childid id memsex age relate livhse grader3 using "$r3yc\in_yc_householdmemberlevel.dta", clear			
			qui append using "$r3oc\in_oc_householdmemberlevel.dta", keep(childid id memsex age relate livhse grader3)
			gen father=1 if relate==1 & memsex==1
			keep if father==1
			...
			rename age dadage
			rename livhse dadlive
			rename grader3 dadedu		
			recode dadedu (14=13) (18=30) (19=15) (77 79 88 99=.)
			keep childid dad*		
			drop if childid=="IN081018" & dadid==0
			g round=3
			tempfile  dad3
			save     `dad3'					
							
	* ROUND 4 (2 duplicates: IN151014 dadid=31 and 32, IN161012 dadid=6)
			use CHILDCODE MEMIDR4 MEMSEXR4 MEMAGER4 RELATER4 GRDE18R4 LIVHSER4 YRDIEDR4 using "$r4yc3\IN_R4_YCHH_HouseholdRosterR4.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_HouseholdRosterR4.dta", keep(CHILDCODE MEMIDR4 MEMSEXR4 MEMAGER4 RELATER4 GRDE18R4 LIVHSER4 YRDIEDR4)
			...
			keep childid dad*
			drop if childid=="IN151014" & dadid==31
			drop if childid=="IN151014" & dadid==32
			drop if childid=="IN161012" & dadid==6
			g round=4
			tempfile  dad4
			save     `dad4'								
				
	* ROUND 5 (13 duplicates)
			...
			recode GRDE18R5 (14=13) (15=14) (16=15) (17=30) (77 79 88 99=.)
			...
			keep childid dad*
			drop if childid=="IN061046" & dadid==31
			drop if childid=="IN071024" & dadid==3
			drop if childid=="IN091007" & dadid==10
			drop if childid=="IN091007" & dadid==11
			drop if childid=="IN091019" & dadid==8
			drop if childid=="IN091019" & dadid==9
			drop if childid=="IN111011" & dadid==30
			drop if childid=="IN111015" & dadid==7
			drop if childid=="IN111051" & dadid==10
			drop if childid=="IN111054" & dadid==12
			drop if childid=="IN121003" & dadid==8
			drop if childid=="IN121048" & dadid==8
			drop if childid=="IN161022" & dadid==8
			drop if childid=="IN191042" & dadid==34
			g round=5
			tempfile  dad5
			save     `dad5'					
	...
	* CORRECTIONS FOR FATHER'S ID (Father should have the same ID across rounds, use MODE)
			...
			****************************************/	
			...
			replace dadid2=5 if childid=="IN110041"
			drop dadid
...
***** PARENT CHARACTERISTICS (MOTHER) *****
	...
	* ROUND 1
			...
			rename id momid
			rename age momage
			rename grading momedu
			keep childid mom*
			tempfile  mom1
			save     `mom1'						

			use childid momlive using "$r1yc\inchildlevel1yrold.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid momlive)
	...
	* ROUND 2 (1 duplicate: IN110017 momid=4)
			use  childid id memsex livhse grade age relate using "$r2yc\insubhouseholdmember5.dta", clear
			qui append using "$r2oc\insubhouseholdmember12.dta", keep(childid id memsex age relate livhse grade)
			...
			drop if childid=="IN110017" & momid==4
			keep childid mom*
			tempfile  mom2
			save     `mom2'

			use childid mumlits using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid mumlits)
	...
	* ROUND 3 (7 duplicates)
			...
			rename age momage
			rename livhse momlive
			rename grader3 momedu		
			recode momedu (14=13) (18=30) (19=15) (77 79 88 99=.)
			keep childid mom*		
			drop if childid=="IN010074" & momid==2
			drop if childid=="IN110017" & momid==4 
			drop if childid=="IN111026" & momid==10 
			drop if childid=="IN111027" & momid==5 
			drop if childid=="IN161016" & momid==3 
			drop if childid=="IN201016" & momid==29 
			drop if childid=="IN201025" & momid==7			
			g round=3
			tempfile  mom3
			save     `mom3'					
							
	* ROUND 4 (5 duplicates)
			use CHILDCODE MEMIDR4 MEMSEXR4 MEMAGER4 RELATER4 GRDE18R4 LIVHSER4 YRDIEDR4 using "$r4yc3\IN_R4_YCHH_HouseholdRosterR4.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_HouseholdRosterR4.dta", keep(CHILDCODE MEMIDR4 MEMSEXR4 MEMAGER4 RELATER4 GRDE18R4 LIVHSER4 YRDIEDR4)
			recode GRDE18R4 (14=13) (15=14) (16=15) (17=30) (77 79 88 99=.)
			recode MEMAGER4 (-88 -77=.)
			...
			keep childid mom*
			drop if childid=="IN070040" & momid==5
			drop if childid=="IN110017" & momid==4
			drop if childid=="IN111027" & momid==5
			drop if childid=="IN151007" & momid==30
			drop if childid=="IN161022" & momid==7
			g round=4
			tempfile  mom4
			save     `mom4'								
				
	* ROUND 5 (7 duplicates)
			use CHILDCODE MEMIDR5 MEMSEXR5 MEMAGER5 RELATER5 GRDE18R5 LIVHSER5 YRDIEDR5 using "$r5ychh\HouseholdRosterR5.dta", clear
			qui append using "$r5ochh\HouseholdRosterR5.dta", keep(CHILDCODE MEMIDR5 MEMSEXR5 MEMAGER5 RELATER5 GRDE18R5 LIVHSER5 YRDIEDR5)
			recode GRDE18R5 (14=13) (15=14) (16=15) (17=30) (77 79 88 99=.)
			recode MEMAGER5 (-88 -77 1 2=.)
			...
			keep childid mom*
			drop if childid=="IN041036" & momid==7
			drop if childid=="IN061023" & momid==8
			drop if childid=="IN061030" & momid==9
			drop if childid=="IN061030" & momid==10			
			drop if childid=="IN111027" & momid==13
			drop if childid=="IN111041" & momid==32
			drop if childid=="IN121048" & momid==9
			drop if childid=="IN161022" & momid==7
			g round=5
			tempfile  mom5
			save     `mom5'					
...
***** CAREGIVER CHARACTERISTICS *****
	...
	* ROUND 1
			use childid careid literany head using "$r1yc\inchildlevel1yrold.dta", clear
			merge 1:m childid using "$r1yc\insubsec2householdroster1.dta", nogen keepusing(childid id age sex relate grading)
			tempfile careyc1
			save `careyc1'
			use childid careid literany head using "$r1oc\inchildlevel8yrold.dta", clear
			merge 1:m childid using "$r1oc\insubsec2householdroster8.dta", nogen keepusing(childid id age sex relate grading)
			qui append using `careyc1'
			keep if id==careid
			recode literany head relate grading (88 99=.)
			recode grading (13/99=.)
			rename age careage
			rename sex caresex
			...
			* Correct relationship of caregiver to child if carerel=13 using round 2 data
			use childid id relate using "$r2yc\insubhouseholdmember5.dta", clear
			qui append using "$r2oc\insubhouseholdmember12.dta", keep(childid id relate)
			recode relate (1=1) (2 3 4=2) (5 6=3) (13=4) (7/12=5) (14/19=6) (20/22=7)			
	...
	* ROUND 2
			use childid careid carelits headid mumid dadid ladder farlad using "$r2yc\inchildlevel5yrold.dta", clear
			merge 1:m childid using "$r2yc\insubhouseholdmember5.dta", nogen keepusing(childid id age memsex relate grade chgrade)
			tempfile careyc2
			save `careyc2'
			use childid careid carelits headid mumid dadid ladder farlad using "$r2oc\inchildlevel12yrold.dta", clear
			merge 1:m childid using "$r2oc\insubhouseholdmember12.dta", nogen keepusing(childid id age memsex relate grade chgrade)
	...
	* ROUND 3 (no info on education of members aged 18 above)
			use childid pridadr3 primumr3 mumidr3 dadidr3 careidr3 headid ladderr3 farladr3 using "$r3yc/in_yc_householdlevel.dta", clear	
			merge 1:m childid using "$r3yc/in_yc_householdmemberlevel.dta", nogen keepusing(childid id age memsex relate grader3)
			tempfile careyc3
			save `careyc3'
			use childid pridadr3 primumr3 mumidr3 dadidr3 careidr3 headid using "$r3oc/in_oc_householdlevel.dta", clear
			merge 1:m childid using "$r3oc/in_oc_householdmemberlevel.dta", nogen keepusing(childid id age memsex relate grader3)
			...
			recode caredu (14=13) (18=30) (19=15) (77 79 88 99=.)
	...
	* ROUND 4 (YC ONLY)
			use CHILDCODE MEMIDR4 RELATER4 MEMSEXR4 MEMAGER4 STRT18R4 GRDE18R4 HSSTRTR4 GRADER4 RELHHR4 PRMCRER4 using "$r4yc3\IN_R4_YCHH_HouseholdRosterR4.dta", clear
			recode GRDE18R4 (14=13) (15=14) (16=15) (17=30) (77 79 88 99=.)
			recode GRADER4 (14=13) (15 16=14) (19=15) (18=30) (77 79 88 99=.)
			replace GRDE18R4=GRADER4 if missing(GRDE18R4)
			replace GRDE18R4=0 if STRT18R4==0 | HSSTRTR4==0
			keep if PRMCRER4=="Y"
			duplicates drop CHILDCODE, force
			merge 1:1 CHILDCODE using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta", nogen keepusing(CHILDCODE HEADIDR4 LADDERR4 FARLADR4)
			...
			replace carehead=2 if RELHHR4==1 & careid !=.
			recode  carehead (.=3) if careid !=.
	...
	* ROUND 5 (YC ONLY)
			...
			recode GRDE18R5 (14=13) (15=14) (16=15) (17=30) (77 79 88 99=.)
			recode GRADER5 (14=13) (15 16=14) (19=15) (18=30) (77 79 88 99=.)
			...
			recode caredu (77 79 88 =.)
			g carehead=1 if HEADIDR5==careid & careid !=.
			replace carehead=2 if RELHHR5==1 & careid !=.
			recode  carehead (.=3) if careid !=.
...
***** HOUSEHOLD WEALTH INDEX AND SUBINDICES *****
	...
	* ROUNDS 1-3
			use childid hq-wi round using "$wealth/R123/in_wi_r123.dta", clear
			tempfile    wealth3
			save       `wealth3'			
	
	* ROUND 4	
			use childid hq-wi using "$wealth/R4/in_r4_oc_wealthindex.dta", clear
			qui append using "$wealth/R4/in_r4_yc_wealthindex.dta", keep(childid hq-wi)
			g round=4
			tempfile   wealth4
			save      `wealth4'
	...
	* ROUND 5
			...
			use "$r5wealth\in_r5_yc_hh_hhsize_wi.dta", clear
			qui append using "$r5wealth\in_r5_oc_hh_hhsize_wi.dta"
			gen childid="IN"+string(CHILDCODE,"%06.0f")
			drop CHILDCODE adeqhhsizer5 hhsize
			rename drwater drwaterq
			rename toilet toiletq
			rename elec elecq
			rename cooking cookingq
			g round=5 
			tempfile wealth5
			save `wealth5'
...
***** LIVESTOCK OWNERSHIP *****
	...
	* ROUND 1    
			use childid animals anyaim* aniown* using "$r1yc/inchildlevel1yrold.dta", clear
			qui append using "$r1oc/inchildlevel8yrold.dta", keep(childid animals anyaim* aniown*)

			* Four groups
			recode aniown* (. -8888 -9999=0)
	...
	* ROUND 2 
			use childid animals anyaim* numaim* using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid animals anyaim* numaim*)
	...
	* ROUND 3
			use childid animalr3 ayanr3* nmamr3* using "$r3yc\in_yc_householdlevel.dta", clear
			qui append using "$r3oc\in_oc_householdlevel.dta", keep(childid animalr3 ayanr3* nmamr3*)
			...
			* Four groups
			...
			gen anirumi =nmamr313+ nmamr314+ nmamr315
			gen anispec =nmamr316+ nmamr317 +nmamr318 +nmamr319 +nmamr320
			...
			* Each type of animal
			...
			gen anibull=nmamr308
			gen anihebu=nmamr307
			gen anidonk=nmamr309
			gen aniybul=nmamr310
			gen anishee=nmamr311
			gen anigoat=nmamr312
			gen anipigs=nmamr313
			gen anipoul=nmamr314
			gen anirabb=nmamr315
			gen anibeeh=nmamr316
			gen anifish=nmamr317
			gen anishri=nmamr318
			gen anifshr=nmamr319
			gen aniothr=nmamr320
	...
	* ROUND 4	
			use AYANR4 NMAMR4 CHILDCODE LVSKIDR4 using "$r4yc3\IN_R4_YCHH_LivelihoodsLivestock.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_LivelihoodsLivestock.dta", keep(AYANR4 NMAMR4 CHILDCODE LVSKIDR4)
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop  CHILDCODE

			gen ayanr4= .
			replace ayanr4=1 if AYANR4=="Y"
			replace ayanr4=0 if AYANR4=="N"
			drop AYANR4
			rename *, lower 

			reshape wide ayanr4 nmamr4, i(childid) j(lvskidr4)
			recode ayanr4* nmamr4* (.=0)
			order childid ayanr4* nmamr4* 
			gen aniany=0
			foreach var of varlist ayanr41 - ayanr420{ 
				replace aniany=1 if `var'==1
				}	
		
			* Four groups			
			gen animilk =nmamr41 	+ nmamr42 	+ nmamr43 	+ nmamr44  + nmamr45  + nmamr46
			gen anidrau =nmamr47 	+ nmamr48 	+ nmamr49 	+ nmamr410 
			gen anirumi =nmamr411 	+ nmamr412	+ nmamr413 	+ nmamr414 	+ nmamr415 	
			gen anispec =nmamr416   + nmamr417  + nmamr418  + nmamr419  + nmamr420
			...
			* each type of animal
			...
			gen anishee=nmamr411
			gen anigoat=nmamr412
			gen anipigs=nmamr413
			gen anipoul=nmamr414
			gen anirabb=nmamr415
			gen anibeeh=nmamr416
			gen anifish=nmamr417
			gen anishri=nmamr418
			gen anifshr=nmamr419
			gen aniothr=nmamr420
	...
	* ROUND 5	
			...
			gen aniany=0
			foreach var of varlist ayanr51-ayanr520{ 
				replace aniany=1 if `var'==1
				}	
		
			* Four groups			
			gen animilk =nmamr51 	+ nmamr52 	+ nmamr53 	+ nmamr54  + nmamr55  + nmamr56
			gen anidrau =nmamr57 	+ nmamr58 	+ nmamr59 	+ nmamr510 
			gen anirumi =nmamr511 	+ nmamr512	+ nmamr513 	+ nmamr514 	+ nmamr515 	
			gen anispec =nmamr516   + nmamr517  + nmamr518  + nmamr519  + nmamr520
			...
			* each type of animal
			...
			gen anishee=nmamr511
			gen anigoat=nmamr512
			gen anipigs=nmamr513
			gen anipoul=nmamr514
			gen anirabb=nmamr515
			gen anibeeh=nmamr516
			gen anifish=nmamr517
			gen anishri=nmamr518
			gen anifshr=nmamr519
			gen aniothr=nmamr520
	...
	* LABELS
			...
			label var anihebu "Number of he-buffalo"
...
***** LAND AND HOUSE OWNERSHIP *****
	...
	* ROUND 1
			use childid ownhouse using "$r1yc/inchildlevel1yrold.dta", clear               
			qui append using "$r1oc/inchildlevel8yrold.dta", keep(childid ownhouse)	
	...
	* ROUND 2
			use childid plotid lown luse using "$r2yc\insubplotsofland5.dta", clear            
			qui append using "$r2oc\insubplotsofland12.dta", keep(childid plotid lown luse)
			...
			use childid ownhouse using "$r2yc\inchildlevel5yrold.dta", clear
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid ownhouse)
	...
	* ROUND 3
			use childid ownhser3 using "$r3yc\in_yc_householdlevel.dta", clear
			qui append using "$r3oc\in_oc_householdlevel.dta", keep(childid ownhser3)
	...
	* ROUND 4
			use CHILDCODE LANDIDR4 LNDLIVR4 USELIVR4 LNDTYPR4 using "$r4yc3\IN_R4_YCHH_LandType.dta", clear		
			qui append using "$r4oc3\IN_R4_OCHH_LandType.dta", keep(CHILDCODE LANDIDR4 LNDLIVR4 USELIVR4 LNDTYPR4) 
			g accomm=1 if LANDIDR4==1 & LNDLIVR4=="Y"
			replace accomm=0 if LANDIDR4!=1 & LNDLIVR4!=""
			...
			use CHILDCODE OWNHSER4 using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_OlderHousehold.dta", keep(CHILDCODE OWNHSER4)
	...
	* ROUND 5 
			use CHILDCODE LANDIDR5 USEINDR5 USERANR5 USEDRYR5 using "$r5ychh\LandType.dta", clear
			recode USEINDR5 USERANR5 USEDRYR5 (77 79 88=.)
			g accomm=LANDIDR5==1 if USEINDR5>=1 & USEINDR5<=4
			bys CHILDCODE: egen ownlandhse=max(accomm)
			keep CHILDCODE ownlandhse
			duplicates drop
			tempfile land5
...
***** COUNTRY-SPECIFIC PUBLIC PROGRAMMES *****
	...
	* ROUND 2 
			use childid dpip inbnpr02 using "$r2yc/inchildlevel5yrold.dta", clear
			qui append using "$r2oc/inchildlevel12yrold.dta", keep(childid dpip inbnpr02)
			rename inbnpr02 ikp
			rename dpip ikp_child
			g round=2
			tempfile program2
			save `program2'
	
	* ROUND 3
			* question on unemployment allowance under NREG asked differently compared with R4 and R5, therefore it was excluded.
			use childid pds* nregegs wrknrgr3 hvrajvr3 bnikpr3 abdpipr3 using  "$r3yc/in_yc_householdlevel.dta", clear
			qui append using "$r3oc/in_oc_householdlevel.dta", keep(childid pds* nregegs wrknrgr3 hvrajvr3 bnikpr3 abdpipr3)
			rename nregegs nregs
			rename wrknrgr3 nregs_work
			rename hvrajvr3 rajiv
			rename bnikpr3 ikp
			rename abdpipr3 ikp_child
			g pds=0 	// 273 nonresponse coded as not receiving pds	
			replace pds=1 if pdsrcer3==1 | pdsdhlr3==1 | pdssgrr3==1| pdskrsr3==1 | pdsoilr3==1 | pdsothr3==1			
			drop pdsrcer3 pdsdhlr3 pdssgrr3 pdskrsr3 pdsoilr3 pdsothr3
			g round=3
			tempfile program3
			save `program3'

	* ROUND 4		
			use CHILDCODE ACCPDSR4 INREGGR4 WRKNRGR4 UNEMALR4 RAJIVR4 RAJGANR4 IKPANYR4 using "$r4yc3/IN_R4_YCHH_YoungerHousehold.dta", clear
			qui append using "$r4oc3/IN_R4_OCHH_OlderHousehold.dta", keep(CHILDCODE ACCPDSR4 INREGGR4 WRKNRGR4 UNEMALR4 RAJIVR4 IKPANYR4)
			recode ACCPDSR4 INREGGR4 WRKNRGR4 UNEMALR4 RAJIVR4 RAJGANR4 IKPANYR4 (77 79 88=.)
			rename ACCPDSR4 pds
			rename INREGGR4 nregs
			recode WRKNRGR4 (.=0) if nregs==0
			rename WRKNRGR4 nregs_work
			recode UNEMALR4 (.=0) if nregs==0 | nregs_work==0
			g nregs_allow=UNEMALR4==2 if UNEMALR4<=2
			rename RAJIVR4 rajiv
			g sabla_yl=RAJGANR4==2 if RAJGANR4<=2
			g ikp=IKPANYR4==2 if IKPANYR4<=2
			g childid="IN"+string(CHILDCODE,"%06.0f")
			drop CHILDCODE UNEMALR4 RAJGANR4 IKPANYR4
			g round=4
			tempfile program4
			save `program4'	
			
	* ROUND 5		
			use CHILDCODE ACCPDSR5 INREGGR5 WRKNRGR5 UNEMALR5 RAJIVR5 SABLAR5 CBNSABR5 IKPANYR5 using "$r5ychh/YoungerHousehold.dta", clear
			rename IKPANYR5 ikp_yc
			qui append using "$r5ochh/OlderHousehold.dta", keep(CHILDCODE ACCPDSR5 INREGGR5 WRKNRGR5 UNEMALR5 RAJIVR5 IKPANYR5)
			recode ACCPDSR5 INREGGR5 WRKNRGR5 UNEMALR5 RAJIVR5 SABLAR5 CBNSABR5 IKPANYR5 (77 79 88=.)
			rename ACCPDSR5 pds
			rename INREGGR5 nregs
			recode WRKNRGR5 (.=0) if nregs==0
			rename WRKNRGR5 nregs_work
			recode UNEMALR5 (.=0) if nregs==0 | nregs_work==0
			g nregs_allow=UNEMALR5==2 if UNEMALR5<=2
			rename RAJIVR5 rajiv
			rename SABLAR5 sabla
			rename CBNSABR5 sabla_yl
			recode sabla_yl (.=0) if sabla==0
			g ikp=IKPANYR5==2 if IKPANYR5<=2 
			replace ikp=ikp_yc if missing(ikp)
			g childid="IN"+string(CHILDCODE,"%06.0f")
			drop CHILDCODE UNEMALR5 IKPANYR5 ikp_yc
			g round=5
			tempfile program5
			save `program5'	
	...
	* LABEL
			lab var pds			"Household is accessing the Public Distribution System"
			lab var nregs		"Household has a job card under the NREGS"
			lab var nregs_work	"At least one member has worked for the NREGS in the past 12 mos"
			lab var nregs_allow	"Household has received unemployment allowance under NREGS since previous round"
			lab var rajiv		"Respondent has Rajiv/NTR Arogyasri card"
			lab var sabla		"At least one member has accessed the SABLA programme since previous round"
			lab var sabla_yl	"YL child has benefitted from the nutritional component under REGSEAG/SABLA"
			lab var ikp			"At least one member has benefitted from IKP-Credit provision since previous round"
			lab var ikp_child	"At least one child of the respondent has benefitted from IKP"
			label define yesno 1 "yes" 0 "no"
			label values pds nregs nregs_work nregs_allow rajiv sabla sabla_yl ikp ikp_child yesno	
			sort childid round
			order childid round pds nregs nregs_work nregs_allow rajiv sabla sabla_yl ikp ikp_child
			tempfile   program
			save      `program'			
...
***** CREDIT AND FOOD SECURITY **** 			
	...
	* ROUND 3
			use childid obtnlnr3 getlnr3 fdhomer3 using "$r3yc/in_yc_householdlevel.dta", clear
			qui append using "$r3oc/in_oc_householdlevel.dta", keep(childid obtnlnr3 getlnr3 fdhomer3)
			recode obtnlnr3 getlnr3 fdhomer3 (77 88 99=.)
	...
	* ROUND 4 (YC ONLY)
			use CHILDCODE OBTNLNR4 GETLNR4 FDHOMER4 using "$r4yc3/IN_R4_YCHH_YoungerHousehold.dta", clear
...
***** HOUSEHOLD HEAD CHARACTERISTICS *****	
	...
	* ROUND 1
			use childid headid using "$r1yc\inchildlevel1yrold.dta", clear			
			merge 1:m childid using  "$r1yc\insubsec2householdroster1.dta", nogen keepusing(childid id age sex relate grading)
			keep if id==headid
			tempfile headinfo1yc
			save `headinfo1yc'
			use childid headid using "$r1oc/inchildlevel8yrold.dta", clear	
			merge 1:m childid using "$r1oc\insubsec2householdroster8.dta", nogen keepusing(childid id age sex relate grading)
	...
	* ROUND 2
			use childid headid using "$r2yc\inchildlevel5yrold.dta", clear	
			merge 1:m childid using "$r2yc\insubhouseholdmember5.dta", nogen keepusing(childid id age memsex relate grade chgrade)
			tempfile headinfo2yc
			save `headinfo2yc'
			use childid headid using "$r2oc/inchildlevel12yrold.dta", clear
			merge 1:m childid using "$r2oc\insubhouseholdmember12.dta", nogen keepusing(childid id age memsex relate grade chgrade)
	...
	* ROUND 3 (no info on education of members aged 18 above)
			use childid headidr3 using "$r3yc\in_yc_householdlevel.dta", clear
			merge 1:m childid using "$r3yc/in_yc_householdmemberlevel.dta", nogen keepusing(childid id age memsex relate grader3)
			tempfile headinfo3yc
			save `headinfo3yc'
			use childid headidr3 using "$r3oc\in_oc_householdlevel.dta", clear
			merge 1:m childid using "$r3oc/in_oc_householdmemberlevel.dta", nogen keepusing(childid id age memsex relate grader3)
			qui append using `headinfo3yc'
			keep if headidr3==id
			rename headidr3 headid
			rename age headage
			rename memsex headsex
			rename relate headrel
			recode headrel (1=1) (2 3 4 24=2) (5 6=3) (13=4) (7/12=5) (14/19 27=6) (20/23=7) (25=8) (26=9)
			rename grader3 headedu
			recode headedu (14=13) (18=30) (19=15) (77 79 88 99=.)
			keep childid head*
			g round=3
			tempfile head3
			save `head3'					
	...
	* ROUND 4 
			use CHILDCODE HEADIDR4 using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta", clear			
			merge 1:m CHILDCODE using "$r4yc3\IN_R4_YCHH_HouseholdRosterR4.dta", nogen ///
						keepusing(CHILDCODE MEMIDR4 RELATER4 MEMSEXR4 MEMAGER4 GRDE18R4 GRADER4 STRT18R4 HSSTRTR4)
			tempfile     headinfo4yc
			save        `headinfo4yc'
			use CHILDCODE HEADIDR4 using "$r4oc3\IN_R4_OCHH_OlderHousehold.dta", clear
			merge 1:m CHILDCODE using "$r4oc3\IN_R4_OCHH_HouseholdRosterR4.dta", nogen ///
						keepusing(CHILDCODE MEMIDR4 RELATER4 MEMSEXR4 MEMAGER4 GRDE18R4 GRADER4 STRT18R4 HSSTRTR4)
	...
	* ROUND 5 
			...
			merge 1:m CHILDCODE using "$r5ychh\HouseholdRosterR5.dta", nogen ///
						keepusing(CHILDCODE MEMIDR5 RELATER5 MEMSEXR5 MEMAGER5 GRDE18R5 GRADER5 STRT18R5 HSSTRTR5)
...
***** HOUSEHOLD SIZE *****
	...
	* ROUND 1
			use childid hhsize using "$r1yc/inchildlevel1yrold.dta", clear
			qui append using "$r1oc/inchildlevel8yrold.dta", keep(childid hhsize)
			gen round=1
			tempfile    hhsize1
			save       `hhsize1'	
		
	* ROUND 2
			use childid hhsize using "$r2yc/inchildlevel5yrold.dta", clear
			qui append using "$r2oc/inchildlevel12yrold.dta", keep(childid hhsize)
			gen round=2
			tempfile    hhsize2
			save       `hhsize2'

	* ROUND 3
			use childid hhsize using "$r3yc/in_yc_householdlevel.dta", clear
			qui append using "$r3oc/in_oc_householdlevel.dta", keep(childid hhsize)
			gen round=3
			tempfile    hhsize3
			save       `hhsize3'

	* ROUND 4
			use CHILDCODE hhsize using "$r4yc3\IN_R4_YCHH_YoungerHousehold.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_OlderHousehold.dta", keep(CHILDCODE hhsize)
			g childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE
			gen round=4
			tempfile    hhsize4
	...
	* ROUND 5
			use CHILDCODE hhsizer5 using "$r5wealth/in_r5_yc_hh_hhsize_wi.dta", clear
			qui append using "$r5wealth/in_r5_oc_hh_hhsize_wi.dta", keep(CHILDCODE hhsizer5)
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE
...
***** SEX AND AGE COMPOSITION OF HOUSEHOLD (excluding YL child) *****

	* ROUND 1
			use childid id age sex using "$r1yc/insubsec2householdroster1.dta", clear
			qui append using "$r1oc/insubsec2householdroster8.dta", keep(childid id age sex)
			tempfile    roster1
			save       `roster1'
				
	* ROUND 2
			use childid id age memsex livhse using "$r2yc/insubhouseholdmember5.dta", clear
			qui append using "$r2oc/insubhouseholdmember12.dta", keep(childid id age memsex livhse)
			keep if livhse==1
			drop if id==0
			rename memsex sex
			tempfile    roster2
	...
	* ROUND 3
			use childid id age memsex livhse using "$r3yc/in_yc_householdmemberlevel.dta", clear
			qui append using "$r3oc/in_oc_householdmemberlevel.dta", keep(childid id age memsex livhse)
			keep if livhse==1
			drop if id==0
			rename memsex sex
			tempfile    roster3
	...
	* ROUND 4
			use CHILDCODE MEMIDR4 MEMAGER4 MEMSEXR4 LIVHSER4 using "$r4yc3/IN_R4_YCHH_HouseholdRosterR4.dta", clear
			qui append using "$r4oc3/IN_R4_OCHH_HouseholdRosterR4.dta", keep(CHILDCODE MEMIDR4 MEMAGER4 MEMSEXR4 LIVHSER4)
			keep if LIVHSER4==1
			drop if MEMIDR4==0
			gen childid="IN"+string(CHILDCODE, "%06.0f")
			drop CHILDCODE
...
***** HOUSEHOLD SHOCKS *****
		...
		* ROUND 1
			...
			use childid phychnge-hhoth using "$r1yc\inchildlevel1yrold.dta", clear
			qui append using "$r1oc\inchildlevel8yrold.dta", keep(childid phychnge-hhoth)
		...
		* ROUND 2
			...
			use childid event* using "$r2yc\inchildlevel5yrold.dta", clear  
			qui append using "$r2oc\inchildlevel12yrold.dta", keep(childid event*)
		...
		* ROUND 3
			...
			use childid evntr* using "$r3yc\in_yc_householdlevel.dta", clear
			qui append using "$r3oc\in_oc_householdlevel.dta", keep(childid evntr*)
		...
		* ROUND 4
			...
			use   "$r4yc3\IN_R4_YCHH_Shocks.dta", clear
			qui append using "$r4oc3\IN_R4_OCHH_Shocks.dta"
...
***** Household Variables *****
			...
			drop if childid=="IN009102" | childid=="IN090109"
			save "$output\india_constructed.dta", replace
